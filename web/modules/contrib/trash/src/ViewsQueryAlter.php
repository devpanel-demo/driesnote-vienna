...
class ViewsQueryAlter implements ContainerInjectionInterface {
  ...
  public function alterQuery(QueryPluginBase $query): void {
    ...
    foreach ($query->relationships as $relationship_table_alias => $info) {
      ...
      $deleted_table_alias = $relationship_table_alias;
      ...
      if ($table_data['table']['entity revision']) {
        ...
        $deleted_table_alias = $query->addTable($data_table, NULL, $join);
      }

      $this->alterQueryForEntityType($query, $entity_type_id, $deleted_table_alias);
    }
  }
